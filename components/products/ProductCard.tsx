...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <Link
      ...
      className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-4 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 cursor-pointer h-full flex flex-col relative group block"
    ...
    >
      <div className="relative w-full h-48 mb-4 bg-slate-100 dark:bg-slate-700 rounded-lg overflow-hidden">
        ...
        <div
          className={`absolute top-2 right-2 z-10 transition-opacity duration-200 ${isHovered ? 'opacity-100' : 'opacity-0'
            }`}
          onClick={(e) => {
            e.preventDefault();
            e.stopPropagation();
